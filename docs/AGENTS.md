...
- 作業ディレクトリの現在地はターミナルプロンプトとセッションサムネイルで把握し、ヘッダーは Tips / 更新案内に使う。
...
- ターミナル上の選択範囲は右クリックでクリップボードへコピーできるようにする。
...
- ヘッダーにはパス表示ではなく Tips を表示し、通常時は60秒ごとまたはクリックで切り替える。更新がある場合は Tips より更新案内を優先表示し、クリックで設定画面へ誘導する。
